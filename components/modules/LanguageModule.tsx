...
export default function LanguageModule({ onBack, onAddCredits }: LanguageModuleProps) {
  ...
  const handleSimulateVoice = () => {
    ...
    setTimeout(() => {
      ...
      onAddCredits(10);
      setAiResponse("Pronunciation check: Excellent tone and flow! +10 Credits unlocked. 🚀");
    }, 2500);
  };
        ...
        <div className="space-y-4 flex flex-col justify-between">
          <div className="bg-white/[0.03] border border-white/10 rounded-[32px] p-6 backdrop-blur-xl space-y-4">
            ...
            <div className="space-y-2">
              {[
                { id: 'casual', label: '☕ Coffee Chat', desc: 'Běžná denní konverzace' },
                { id: 'interview', label: '💼 Job Interview', desc: 'Formální firemní angličtina' },
                { id: 'slang', label: '🔥 Native Slang', desc: 'Hovorové fráze z ulice' },
              ].map((mode) => (
                <button
                  key={mode.id}
                  onClick={() => {
