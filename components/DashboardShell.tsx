...
// ... (zde zůstávají lazy importy modulů jako dřív)
...
function WorkspaceView({ setActiveView, openCustomizer }: { setActiveView: (v: string) => void, openCustomizer: () => void }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Dynamické vykreslování widgetů podle uživatelského nastavení */}
        ...
        <Suspense fallback={<LoadingSkeleton />}>
          {(() => {
            switch (activeView) {
              ...
              case 'language': return <LanguageModule onBack={() => setActiveView('workspace')} onAddCredits={() => {}} />;
              case 'storage': return <StorageModule onBack={() => setActiveView('workspace')} />;
              case 'rewards': return <RewardsModule onBack={() => setActiveView('workspace')} credits={userCredits} onAddCredits={() => {}} />;
              case 'store': return <StoreModule onBack={() => setActiveView('workspace')} userCredits={userCredits} onAddCredits={() => {}} />;
              default: return null;
            }
          })()}
        </Suspense>
